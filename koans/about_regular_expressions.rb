...
class AboutRegularExpressions < EdgeCase::Koan
  ...
  def test_a_vertical_pipe_means_or
    ...
    assert_equal "Summer Gray"[grays, 1], "Summer Gray"[grays, 1]
  ...
  def test_sub_is_like_find_and_replace
    assert_equal "one two-three".sub(/(t\w*)/) { $1[0, 1] }, "one two-three".sub(/(t\w*)/) { $1[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal "one two-three".gsub(/(t\w*)/) { $1[0, 1] }, "one two-three".gsub(/(t\w*)/) { $1[0, 1] }
  end
end
